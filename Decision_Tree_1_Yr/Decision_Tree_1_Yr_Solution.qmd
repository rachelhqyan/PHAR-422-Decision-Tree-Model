---
title: "Healthy_Unhealthy_Decision_Tree_(1_Year_Time_Horizon)"
format: md
editor: visual
---
...
![Decision_Tree_1_Yr](../Figures/1_Yr_Decision_Tree.png)
...
Conventional management (T0) has no initial cost so C_T0 is 0. Every patient enters the model in a Healthy state. From this root node patients may remain Healthy, become Unhealthy, or Die in the first-year, with the Healthy probability equal to 1 minus Unhealthy and Death.
...
The expected cost under conventional management (T0) is `r scales::dollar(sum(exp_cost_T0_1), accuracy = 0.01)`.

The expected cost under treatment (T1) is `r scales::dollar(sum(exp_cost_T1_1), accuracy = 0.01)`.

**\*\*Question: What is the expected QALYs of patients in each strategy?**

The expected QALYs of patients under conventional management (T0) is `r round(sum(exp_qaly_T0_1), 3)`.

The expected QALYs of patients under treatment (T1) is `r round(sum(exp_qaly_T1_1), 3)`.

**\*\*Question: What is the 1-year-cost-effectiveness of the new treatment?**

The 1-year-cost-effectiveness of the new treatment (T1) is `r scales::dollar(icer_1, accuracy = 1)` per QALY gained.
